...
{
  stock_t *stock_information = calloc(1, sizeof(stock_t));
...
{
  merch_t *merch = ioopm_hash_table_lookup(warehouse, (elem_t) { .str_value = name}).element.p;
...
void warehouse_add_merch_with_fields(ioopm_hash_table_t *warehouse, char *name, char *description, int price, ioopm_list_t *locations)
{
  ...
  merch_t *merch_to_add = calloc(1, sizeof(merch_t));
