...
bool ioopm_int_eq(elem_t x, elem_t y) { return x.i == y.i; }
bool ioopm_unsigned_int_eq(elem_t x, elem_t y) { return x.u == y.u; }
bool ioopm_bool_eq(elem_t x, elem_t y) { return x.b == y.b; }
bool ioopm_float_eq(elem_t x, elem_t y) { return x.f == y.f; }
...
ioopm_list_t *ioopm_linked_list_create(ioopm_eq_function eq)
{
  ...
  list->last = NULL;
  ioopm_link_t *first = link_create(list->last, (elem_t){ .p = NULL });
  list->first = first;
  list->eq_func = eq;
  return list;
}
...
static bool linked_list_all_any_helper(bool b, ioopm_list_t *list, ioopm_char_predicate prop, void *extra)
{
  ioopm_link_t *cursor = list->first->next;
